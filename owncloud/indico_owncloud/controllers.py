...
class AddAttachmentOwncloudMixin:
    ...
    def _process(self):
        ...
        if form.validate_on_submit():
            files = form.data['owncloud_file_picker']['files']
            token = form.data['owncloud_file_picker']['token']
            headers = {'Authorization': f'Bearer {token}'}
            folder = form.data['folder'] or AttachmentFolder.get_or_create_default(linked_object=self.object)
            for f in files:
                filename = secure_client_filename(f.split('/')[-1])
                attachment = Attachment(folder=folder, user=session.user, title=filename,
                ...
                with requests.get(f, headers=headers) as r:
...
class RHOwncloudAuthCallback(RH):
    def _process(self):
        callback_script = current_plugin.manifest['callbacks.js']
        return render_plugin_template('owncloud-auth-callback.html', callback_script=callback_script)


class RHOwncloudAuthRenewCallback(RH):
    def _process(self):
        callback_script = current_plugin.manifest['callbacks.js']
        return render_plugin_template('owncloud-auth-renew-callback.html', callback_script=callback_script)
